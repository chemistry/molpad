...
export const clearCommandsReducer: Reducer<StoreState> = (state, action) => {
  switch (action.type) {
    case MOUSE_DOWN: {
      ...
      let newMolecule: JMolWrapModel = structuredClone(molecule);
...
function removeAtom(molecule: JMolWrapModel, atom: JMolAtomWrap) {
  ...
  const bondIdsToDelete = Object.keys(molecule.bonds).filter((bondId) => {
