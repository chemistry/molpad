...
export const fragmentCommandsReducer: Reducer<StoreState> = (state, action) => {
  switch (action.type) {
    case MOUSE_DOWN: {
      const templateId = state.toolbar.type;
      const template = TemplatesHelperService.getTemplateById(templateId)!;
...
function applyTemplateToBond(state: StoreState, template: JMol, bond: JMolBondWrap): StoreState {
  ...
  try {
    ...
    mol.load(molecule, MoleculeDataFormat.jnmol);
    const bViews: Record<string, number> =
      (
        mol as unknown as { getBondsViewClasification(): Record<string, number> }
      ).getBondsViewClasification() ?? {};
    bView = bViews[bond.id] ?? 0;
  } catch {
    // Molecule load error - default bView to 0
  }
  bView = ((template as unknown as { order?: number }).order ?? 1) * bView;
...
function applyTemplateToAtom(state: StoreState, _template: JMol): StoreState {
  return state;
}
...
function applyJMolAndMergeAtoms(state: StoreState, template: JMol) {
  ...
  const atomsReplacementList: Record<string, string> = {};
  ...
  Object.keys(MoleculeToApply.bonds).forEach((bondId) => {
    ...
    if (atom1ToReplace && atom2ToReplace && isConneted(atom1ToReplace, atom2ToReplace, molecule)) {
      // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
