...
export const bondCommandsReducer: Reducer<StoreState> = (state, action) => {
  switch (action.type) {
    case MOUSE_DOWN:
...
function proposeNewBond(state: StoreState) {
  ...
  const newMolecule: JMolWrapModel = structuredClone(molecule);
...
function updateBond(state: StoreState) {
  ...
  const newMolecule = structuredClone(state.data.molecule);
  ...
  const hoveredAtoms = ProjectionHelperService.getProjectionAtoms({
    ...
    return !(atom.id === atom1Id || (cursorData.isAtom2Created && atom.id === atom2Id));
  });
  ...
  if (hoveredAtoms.length !== 0) {
    ...
    const newCoords = MoleculeCalcsHelper.proposeFixedAtom(
      cursorData.startAtom,
      unproject(currentPos)
    );
...
function incrementBondValency(bond: JMolBondWrap, bondOrder: number): JMolBondWrap {
  const newBond: JMolBondWrap = structuredClone(bond);
...
function replaceBond(molecule: JMolWrapModel, bond: JMolBondWrap): JMolWrapModel {
  return {
    ...molecule,
    bonds: {
      ...molecule.bonds,
...
function incrementBondDublicates(state: StoreState): StoreState {
  ...
  let newMolecule = structuredClone(state.data.molecule);
  // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
