...
export class MoleculeHelperService {
  ...
  public static wrapMolecule(molecule: JMol): JMolWrapModel {
    const wrapedAtoms = molecule.atoms.map((atom) => MoleculeHelperService.wrapAtom(atom));
    return {
      id: molecule.id,
  ...
  public static unWrapMolecule(molecule: JMolWrapModel): JMol | undefined {
  ...
  private static wrapAtomList(atoms: JMolAtomWrap[]): JMolAtomWrapCollection {
    return atoms.reduce<JMolAtomWrapCollection>((acc, wraped: JMolAtomWrap) => {
  ...
  private static wrapBondList(
    bonds: JMolBond[],
    atomsWraped: JMolAtomWrap[]
  ): JMolBondWrapCollection {
    return bonds.reduce<JMolBondWrapCollection>((acc, curr: JMolBond) => {
  ...
  private static unWrapAtom(atom: JMolAtomWrap): JMolAtom {
    return [atom.x, atom.y, atom.z, atom.type];
  }
  ...
  private static unWrapBond(bond: JMolBondWrap, atomIdsToUnWrap: string[]): JMolBond {
    ...
    return [atom1Idx, atom12dx, bond.order];
  }
}
